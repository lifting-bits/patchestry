...
def render_op(data: Dict, operations: Dict[str, Dict], functions: Dict[str, Dict], types: Dict[str, Dict], inline=False):
    ...
    if mnemonic.startswith("DECLARE_"):
        if inline:
            ...
            decl = render_typed_var(" " + var_name(data), data["type"], types)
            print(f"{decl}<BR />", end='')
        return
    ...
    if "inputs" in data:
        for input in data["inputs"]:
            print("", end=sep)
            render_input(input, operations, functions, types)
            sep = ", "

    if "condition" in data:  # For a CBRANCH
        render_input(data["condition"], operations, functions, types)
...
def render_block(key: int, data: Dict, operations: Dict[str, Dict], functions: Dict[str, Dict], types: Dict[str, Dict]):
    ...
    for op_key in data["ordered_operations"]:
        ...
        render_op(last_op, operations, functions, types)
    print(f"</TD></TR></TABLE>>];")
...
def render_function(func_key: str, functions: Dict[str, Dict], types: Dict[str, Dict]):
    ...
    for block_key, block in basic_blocks.items():
        render_block(ID[block_key], block, operations, functions, types)
...
def render_functions(data: Dict):
    ...
    for func_key in functions.keys():
        render_function(func_key, functions, data["types"])
    print("}")
