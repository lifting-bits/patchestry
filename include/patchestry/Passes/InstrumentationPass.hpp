...
namespace patchestry::passes {
    ...
    /**
     * @brief Configuration options for controlling function inlining behavior.
     *
     * This structure contains options that control whether and how function inlining
     * is performed during the instrumentation process. When inlining is enabled,
     * patch functions may be inlined at their call sites for better performance.
     */
    struct InlineOptions
    {
     ...
     *
     * The pass operates on CIR (Clang Intermediate Representation) dialect operations
     * within MLIR modules and can optionally inline patch functions for performance.
     */
        ...
        : public mlir::PassWrapper< InstrumentationPass, mlir::OperationPass< mlir::ModuleOp > >
    {
      ...
      public:
         ...
         * 1. Collect all functions and matching operations
         * 2. Instrument function calls within each function
         * 3. Instrument matching operations
         * 4. Optionally inline inserted patch calls
         */
         ...
         * before/after patch modes (replace mode is not supported for operations).
         *
