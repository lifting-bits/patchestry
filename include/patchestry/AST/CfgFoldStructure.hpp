...
namespace patchestry::ast {
    ...
    namespace detail {
        ...
        struct CNode {
            ...
            std::string original_label;             // immutable — set once in BuildCGraph, never cleared
            std::vector<clang::Stmt *> stmts;
            clang::Expr *branch_cond = nullptr;
            bool is_conditional = false;

            /// Terminal control-flow stmt (goto/if-goto) popped by
            /// resolveEdges.  Stored separately from content stmts
            /// (Ghidra-style: operations and edges are decoupled).
            /// Used to reconstruct gotos when fold rules collapse
            /// blocks with unhandled external exits.
            ...
            // Hierarchical block type (Phase 3)
            enum class BlockType : uint8_t {
                kBasic,      // leaf block (original CfgBlock)
            ...
            std::vector<size_t> children;  // child CNode ids absorbed by this block
