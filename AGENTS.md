...
|---|---|---|---|
| `patchir-decomp` | `patchir-decomp` | `tools/patchir-decomp/` | decompile Ghidra JSON into CIR/LLVM/asm/object outputs; re-enter printed or refined C with `-from-c` and check it against the P-Code with `-validate-pcode` |
...
2. Decomp stage:
   ...
   later patching.  `-from-c` re-enters a printed or refined C translation
   unit as the same `ast::TranslationUnit`, optionally checked against the
