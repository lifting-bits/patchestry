...
namespace {
    ...
    const llvm::cl::opt< std::string > from_c_filename( // NOLINT(cert-err58-cpp)
        "from-c",
        llvm::cl::desc(
            "Compile a C translation unit carrying patchestry:tu and function-begin "
            "markers (written by the LLM stage or by hand) instead of lifting JSON.  "
            "The target comes from -target-lang, else the `patchestry:tu` header, "
            "else -input."
        ),
