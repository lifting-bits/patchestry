...
namespace patchestry::codegen {
    ...
    namespace {
        ...
        std::string get_path_to_source(clang::ASTContext *actx) {
        ...
        mlir::Location make_loc_name(mlir::MLIRContext *mctx, clang::ASTContext *actx) {
    ...
    MetaGen::MetaGen(clang::ASTContext *actx, mlir::MLIRContext *mctx, const LocationMap &locs)
        : actx(actx), mctx(mctx), locations(locs) {}

    void *MetaGen::raw_pointer(const clang::Decl *decl) const {
        return static_cast< void * >(const_cast< clang::Decl * >(decl));
    }

    void *MetaGen::raw_pointer(const clang::Stmt *stmt) const {
        return static_cast< void * >(const_cast< clang::Stmt * >(stmt));
    }

    void *MetaGen::raw_pointer(const clang::Expr *expr) const {
        return static_cast< void * >(const_cast< clang::Expr * >(expr));
    }

    mlir::Location MetaGen::location(const clang::Decl *decl) const {
        return location(raw_pointer(decl), decl->getLocation());
    }

    mlir::Location MetaGen::location(const clang::Stmt *stmt) const {
        return location(raw_pointer(stmt), stmt->getBeginLoc());
    }

    mlir::Location MetaGen::location(const clang::Expr *expr) const {
        return location(raw_pointer(expr), expr->getExprLoc());
    }

    uint64_t MetaGen::address_from_location(const std::string &str, char delimiter) const {
    ...
    mlir::Location MetaGen::location(void *data, const clang::SourceLocation &loc) const {
