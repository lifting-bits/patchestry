...
// Simple mutability tracking using a hash table
typedef struct mutability_entry {
    void* ptr;
    size_t size;
    bool is_mutable;
    struct mutability_entry* next;
} mutability_entry_t;

#define MUTABILITY_TABLE_SIZE 256
static mutability_entry_t* mutability_table[MUTABILITY_TABLE_SIZE] = {0};

static size_t hash_ptr(void* ptr) {
    uintptr_t addr = (uintptr_t)ptr;
    return (addr >> 3) % MUTABILITY_TABLE_SIZE; // Simple hash
}
