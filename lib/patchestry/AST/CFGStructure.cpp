...
namespace patchestry::ast {
    ...
    // siblings in a body vector.  Null statements are dropped.
    ...
    void CFGStructure::StructureAll() {
        ...
        //
        // Check recursively: the label might be inside a nested body
    ...
    // Action:  Concatenate sequences, collapse via IdentifyInternal.
    // ---------------------------------------------------------------
    ...
    // strips terminals from interior nodes, and returns the body vector.
    // ---------------------------------------------------------------
    ...
    //
    // Walks body vectors looking for SGoto children whose target SLabel
    // is a sibling in the same sequence and is only referenced by that
    ...
    // target label live in *different* body vectors.  When a label has
    ...
    namespace {
        ...
        /// the enclosing body vector and the label's index within it.
        /// Only labels that are *direct children of a body vector* are
        /// returned — labels embedded as bodies of if/while/switch or as
        /// the sole child of an SLabel do not qualify.
        ///
        /// The body-vector-position restriction is intentional and
        /// load-bearing:
    ...
    // ---------------------------------------------------------------
    // RemoveDeadSSeqChildren — strip unreachable body-vector children
    //
    // Walk body vectors bottom-up.  When child[i] always terminates
    ...
    namespace {
        ...
        /// — the previous implementation only descended through
        /// sequences and SIfThenElse, which was a latent under-scan that could
        ...
        ///
        /// `SNodeAlwaysTerminates` marks any sequence whose last child
        /// terminates as a "terminator" — but after AbsorbFallthroughIntoElse
        /// or similar post-passes move content between siblings, a sequence
        ...
        /// Sequences / loops / switch are still excluded because their
    ...
    // (or a body vector whose last child is SReturn with no other control
