        ...
        void ResolveEdges(BlockSplitter &bs) {
            ...
            for (size_t i = 0; i < bs.blocks.size(); ++i) {
                ...
                if (IsConditionalGoto(last, cond, then_target, else_target)) {
                    ...
                    blk.terminal = last;  // preserve for goto emission
                    blk.stmts.pop_back(); // remove from content stmts
                    ...
                    if (!else_target.empty()) {
                        if (auto tgt = find_target(else_target)) {
                            ...
                            LOG(WARNING) << "ResolveEdges: unresolved else target '"
                                         << else_target << "' in block " << i
                                         << " — edge dropped\n";
                            assert(false && "ResolveEdges: else target not found — control flow lost");
                        }
                    ...
                    if (auto tgt = find_target(then_target)) {
                        ...
                        LOG(WARNING) << "ResolveEdges: unresolved goto target '"
                                     << then_target << "' in block " << i
                                     << " — edge dropped\n";
                        assert(false && "ResolveEdges: goto target not found — control flow lost");
                    }
                    continue;
                }
    ...
    void PopulateSwitchMetadata(Cfg &cfg, const ghidra::Function &func) {
        ...
        for (const auto &[bb_key, bb] : func.basic_blocks) {
            for (const auto &op_key : bb.ordered_operations) {
                ...
                // If BRANCHIND has successor_blocks but no switch_cases,
                // synthesize cases with sequential values (0, 1, 2, ...).
                auto effective_cases = op.switch_cases;
                if (effective_cases.empty() && !op.successor_blocks.empty()) {
                    for (size_t i = 0; i < op.successor_blocks.size(); ++i) {
                        effective_cases.push_back(ghidra::SwitchCase{
                            static_cast<int64_t>(i),
                            op.successor_blocks[i],
                            false
                        });
                    }
                }
                if (effective_cases.empty()) continue;
                ...
                for (const auto &sc : effective_cases) {
                    ...
                    if (it == target_to_succ_idx.end()) {
                        auto lbl_it = label_to_idx.find(target_label);
                        if (lbl_it != label_to_idx.end()) {
                            size_t target_idx = lbl_it->second;
                            size_t new_si = blk.succs.size();
                            blk.succs.push_back(target_idx);
                            target_to_succ_idx[target_label] = new_si;
                            it = target_to_succ_idx.find(target_label);
                        } else {
                            LOG(WARNING) << "PopulateSwitchMetadata: switch case target '"
                                         << sc.target_block
                                         << "' not found in CFG for block '"
                                         << bb_key << "'\n";
                            continue;
                        }
                    }
