...
namespace patchestry::ast {
    ...
    clang::Stmt *
    CleanupStmtTree(clang::ASTContext &ctx, clang::Stmt *s, const std::string &continue_label) {
        if (!s) { return nullptr; }
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(s)) {
            ...
            if (ifs->getElse()) {
                ifs->setElse(
                    PushLabelInside(ctx, CleanupStmtTree(ctx, ifs->getElse(), continue_label))
                );
            }
            return s;
        }
                ...
                else if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(cleaned))
                {
                    if (auto *lcs = llvm::dyn_cast< clang::CompoundStmt >(ls->getSubStmt())) {
                        ...
                        if (it != lcs->body_end()) {
                            ...
                            for (++it; it != lcs->body_end(); ++it) { children.push_back(*it); }
                        } else {
                            children.push_back(ls);
                        }
                    } else {
                        ...
                        std::function< void(clang::Stmt *, unsigned) > findLabel =
                            [&](clang::Stmt *st, unsigned depth) {
                                ...
                                if (auto *gs = llvm::dyn_cast< clang::GotoStmt >(st)) {
                                    assert(
                                        gs->getLabel() && "clang::GotoStmt missing target label"
                                    );
                                ...
                                if (auto *ls2 = llvm::dyn_cast< clang::LabelStmt >(st)) {
                                    assert(ls2->getDecl() && "LabelStmt has null LabelDecl");
                                    if (ls2->getDecl()->getName().str() == common) {
                                        target_decl = ls2->getDecl();
                                    }
                                }
                                for (auto *c : st->children()) { findLabel(c, depth + 1); }
                            };
    ...
    clang::Stmt *RemoveDeadLabels(
        ...
        if (auto *cs = llvm::dyn_cast< clang::CompoundStmt >(s)) {
            ...
            // be removed.  CaseStmt/DefaultStmt are also reachable
            // entry points (via switch dispatch), so a sibling case
            // following a terminating case must not be erased.
            ...
            for (size_t i = 1; i < children.size();) {
                ...
                bool has_reachable_entry                             = false;
                std::function< void(clang::Stmt *, unsigned) > check = [&](clang::Stmt *st,
                                                                           unsigned depth) {
                    if (!st || has_reachable_entry) { return; }
                    if (depth > 256) { return; }
                    if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(st)) {
                        if (live.count(ls->getDecl())) { has_reachable_entry = true; }
                    }
                    if (llvm::isa< clang::CaseStmt >(st) || llvm::isa< clang::DefaultStmt >(st))
                    {
                        has_reachable_entry = true;
                    }
    ...
    clang::Stmt *RemoveEmptyBlocks(clang::ASTContext &ctx, clang::Stmt *s) {
        ...
        auto contains_decl_stmt = [](clang::Stmt *stmt) {
            std::function< bool(clang::Stmt *, unsigned) > walk = [&](clang::Stmt *cur,
                                                                      unsigned depth) -> bool {
                if (!cur) { return false; }
    ...
    clang::Stmt *StripGotoToFollowingLabelFromTailPosition(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(st)) {
            unsigned before = stripped;
            ifs->setThen(
                StripGotoToFollowingLabelFromTailPosition(ctx, ifs->getThen(), target, stripped)
            );
            if (ifs->getElse()) {
    ...
    bool StraightLineContinuationIsSafeToMove(clang::Stmt *st) {
        if (!st) { return true; }
        if (!ContinuationIsSafeToMoveBeforeJoin(st) || llvm::isa< clang::GotoStmt >(st)
            || llvm::isa< clang::CaseStmt >(st) || llvm::isa< clang::DefaultStmt >(st))
        {
            return false;
        }
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(st)) {
            if (ifs->getInit() || ifs->getConditionVariable()) { return false; }
        }
        for (clang::Stmt *child : st->children()) {
    ...
    size_t CountStraightLineStmtUnits(clang::Stmt *stmt) {
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            size_t count  = 1;
            count        += CountStraightLineStmtUnits(ifs->getThen());
            count        += CountStraightLineStmtUnits(ifs->getElse());
            return count;
        }
    ...
    clang::Stmt *CloneStraightLineStmt(clang::ASTContext &ctx, clang::Stmt *stmt) {
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ...
            clang::Stmt *then_copy = CloneStraightLineStmt(ctx, ifs->getThen());
            if (!then_copy) { return nullptr; }
            clang::Stmt *else_copy = nullptr;
            if (ifs->getElse()) {
                else_copy = CloneStraightLineStmt(ctx, ifs->getElse());
        ...
        if (auto *expr = llvm::dyn_cast< clang::Expr >(stmt)) { return CloneExpr(ctx, expr); }
        return nullptr;
    }
    ...
    clang::Stmt *
    CloneContinuationSeq(clang::ASTContext &ctx, const std::vector< clang::Stmt * > &stmts) {
        std::vector< clang::Stmt * > cloned;
    ...
    unsigned CountTailFallthroughLeaves(clang::Stmt *st, llvm::StringRef skip_target) {
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(st)) {
            ...
            count +=
                ifs->getElse() ? CountTailFallthroughLeaves(ifs->getElse(), skip_target) : 1U;
            return count;
        }
        return 1;
    }
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(st)) {
            ...
            if (ifs->getElse()) {
                ifs->setElse(RewriteExternalTargetGotoWithContinuation(
                    ctx, ifs->getElse(), target, continuation, removed_gotos, inserted, failed
                ));
                return st;
            }
    ...
    clang::Stmt *FoldExternalTargetContinuations(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ifs->setThen(FoldExternalTargetContinuations(ctx, ifs->getThen(), target, folded));
            if (ifs->getElse()) {
                ifs->setElse(
        ...
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(stmt)) {
            ws->setBody(FoldExternalTargetContinuations(ctx, ws->getBody(), target, folded));
            return ws;
        }
        if (auto *ds = llvm::dyn_cast< clang::DoStmt >(stmt)) {
            ds->setBody(FoldExternalTargetContinuations(ctx, ds->getBody(), target, folded));
            return ds;
        }
        if (auto *fs = llvm::dyn_cast< clang::ForStmt >(stmt)) {
            fs->setBody(FoldExternalTargetContinuations(ctx, fs->getBody(), target, folded));
            return fs;
        }
        if (auto *sw = llvm::dyn_cast< clang::SwitchStmt >(stmt)) {
            sw->setBody(FoldExternalTargetContinuations(ctx, sw->getBody(), target, folded));
            return sw;
        }
        if (auto *case_stmt = llvm::dyn_cast< clang::CaseStmt >(stmt)) {
            case_stmt->setSubStmt(
                FoldExternalTargetContinuations(ctx, case_stmt->getSubStmt(), target, folded)
            );
            return case_stmt;
        }
        if (auto *default_stmt = llvm::dyn_cast< clang::DefaultStmt >(stmt)) {
            default_stmt->setSubStmt(
                FoldExternalTargetContinuations(ctx, default_stmt->getSubStmt(), target, folded)
            );
            return default_stmt;
        }
        ...
        while (local_changed) {
            ...
            for (size_t i = 0; i + 1 < body.size(); ++i) {
                ...
                if (target_gotos == 0 || CountAllGotos(body[i]) != target_gotos) { continue; }

                const unsigned fallthroughs = CountTailFallthroughLeaves(body[i], target);
                if (fallthroughs == 0 || fallthroughs > kMaxExternalContinuationFallthroughs) {
                    continue;
                }
                ...
                if (failed || removed != target_gotos || inserted != fallthroughs) { continue; }
    ...
    bool TryMoveTailContinuationIntoDispatch(
        clang::ASTContext &ctx, std::vector< clang::Stmt * > &body, llvm::StringRef next_label,
        const std::unordered_set< clang::LabelDecl * > *live
    ) {
    ...
    clang::Stmt *ReplaceTrailingDeepStmt(
        clang::ASTContext &ctx, clang::Stmt *st, clang::Stmt *target, clang::Stmt *replacement
    ) {
        ...
        if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(st)) {
            ls->setSubStmt(ReplaceTrailingDeepStmt(ctx, ls->getSubStmt(), target, replacement));
            return ls;
        }
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(st)) {
            if (DeepTrailingStmt(ifs->getThen()) == target) {
                ifs->setThen(ReplaceTrailingDeepStmt(ctx, ifs->getThen(), target, replacement));
                return ifs;
            }
            if (ifs->getElse() && DeepTrailingStmt(ifs->getElse()) == target) {
                ifs->setElse(ReplaceTrailingDeepStmt(ctx, ifs->getElse(), target, replacement));
                return ifs;
            }
        }
        return st;
    ...
    clang::IfStmt *BuildIfGotoArmReplacement(
        ...
        if (arm == 1) {
            ...
            return clang::IfStmt::Create(
                ...
                loc, loc, ifs->getThen(), loc, new_else
            );
        }
    ...
    clang::Stmt *ProcessCompound(
        ...
        while (local_changed) {
            ...
            for (size_t i = 0; i + 1 < body.size(); ++i) {
                ...
                auto *external_folded =
                    FoldExternalTargetContinuations(ctx, body[i], next_label, folded_external);
        ...
        for (size_t i = 0; i + 1 < body.size(); ++i) {
            ...
            std::vector< clang::Stmt * > sw_stmts(sw_body->body_begin(), sw_body->body_end());
    ...
    bool ClangScopeifyLabelsAreRegionLocal(
        ...
        std::function< void(clang::Stmt *, unsigned) > collect_labels = [&](clang::Stmt *stmt,
                                                                            unsigned depth) {
            if (!stmt) { return; }
            if (depth > 256) { return; }
            ...
            for (clang::Stmt *child : stmt->children()) { collect_labels(child, depth + 1); }
        };
    ...
    clang::Stmt *ScopeifyIfGotos(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(s)) {
            ...
            if (ifs->getElse()) { ifs->setElse(ScopeifyIfGotos(ctx, ifs->getElse(), refs)); }
            return s;
        }
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(s)) {
        ...
        while (changed) {
            ...
            for (size_t i = 0; i < body.size(); ++i) {
                    ...
                    if (next_label) {
                            ...
                            && !IsEffectivelyEmptyStmt(ifs->getElse()))
                        {
                            replacement = clang::IfStmt::Create(
                                ...
                                NegateExpr(ctx, ifs->getCond()), loc, loc, ifs->getElse(), loc,
                                nullptr
                            );
                        } else if (
                ...
                auto *tail_goto =
                    llvm::dyn_cast_or_null< clang::GotoStmt >(DeepTrailingStmt(ifs->getThen()));
    ...
    void CountGotoDeclRefs(
        ...
        for (clang::Stmt *child : stmt->children()) { CountGotoDeclRefs(child, refs, seen); }
    }
    ...
    void CountGotoDeclRefs(
        ...
        // Coalesce counts by label name: a clone pass can split same-named gotos
        // across distinct LabelDecls, so a decl-keyed count under-counts a label
        // and a refs-based fold would erase it while a same-named goto survives.
        // Give every decl the combined count so such folds stay conservative.
    ...
    clang::Stmt *ExtractFirstNestedTargetLabel(
        ...
        const std::unordered_set< clang::LabelDecl * > &targets, bool under_structured_scope,
        ExtractedNestedLabel &extracted, bool &changed
    ) {
        ...
        if (auto *label = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            ...
            label->setSubStmt(ExtractFirstNestedTargetLabel(
                ctx, label->getSubStmt(), targets, under_structured_scope, extracted, changed
            ));
            return label;
        }
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ...
            if (ifs->getElse()) {
                ifs->setElse(ExtractFirstNestedTargetLabel(
                    ctx, ifs->getElse(), targets, /*under_structured_scope=*/true, extracted,
                    changed
                ));
            }
            return ifs;
        ...
        if (auto *compound = llvm::dyn_cast< clang::CompoundStmt >(stmt)) {
            ...
            for (size_t k = 0; k < body.size(); ++k) {
                ...
                if (under_structured_scope && lbl && targets.contains(lbl->getDecl())) {
                    std::vector< clang::Stmt * > run;
                    if (auto *sub_c =
                            llvm::dyn_cast_or_null< clang::CompoundStmt >(lbl->getSubStmt()))
                    {
                    ...
                    size_t m = k + 1;
                    while (m < body.size() && !llvm::isa< clang::LabelStmt >(body[m])) {
                    ...
                    if (run_safe) {
                        ...
                        extracted.body = run.empty()
                            ? static_cast< clang::Stmt * >(new (ctx)
                                                               clang::NullStmt(VirtualLoc(ctx)))
                            : run.size() == 1 ? run.front()
                    ...
                    if (lbl->getSubStmt()) {
                        lbl->setSubStmt(ExtractFirstNestedTargetLabel(
                            ctx, lbl->getSubStmt(), targets, under_structured_scope, extracted,
                            changed
                        ));
                    }
                ...
                children.push_back(ExtractFirstNestedTargetLabel(
                    ctx, body[k], targets, under_structured_scope, extracted, changed
                ));
                if (changed) {
                    for (size_t r = k + 1; r < body.size(); ++r) {
                        children.push_back(body[r]);
    ...
    clang::LabelDecl *CreateSyntheticJoinLabel(
        ...
        auto *decl = clang::LabelDecl::Create(ctx, fn, VirtualLoc(ctx), &ctx.Idents.get(name));
        if (fn) {
    ...
    clang::Stmt *HoistCrossScopeLabelEntries(
        ...
        while (changed) {
            ...
            for (size_t i = 0; i < body.size(); ++i) {
                ...
                injected.push_back(
                    new (ctx) clang::LabelStmt(VirtualLoc(ctx), extracted.decl, extracted.body)
                );
    ...
    clang::Stmt *RepairCrossScopeLabelEntries(
        ...
        while (changed) {
            ...
            for (size_t i = 0; i + 1 < body.size(); ++i) {
                ...
                for (size_t j = i + 1; j <= carrier_idx; ++j) { else_body.push_back(body[j]); }
    ...
    clang::Stmt *ReplaceGotoToSwitchLocalLabel(
        ...
        if (auto *compound = llvm::dyn_cast< clang::CompoundStmt >(stmt)) {
            ...
            for (clang::Stmt *child : compound->body()) {
                clang::Stmt *rewritten =
                    ReplaceGotoToSwitchLocalLabel(ctx, child, target, replacement, replaced);
    ...
    // Multi-ref terminal/error labels are the dominant residual goto
    // bucket after the earlier structuring passes.  Keep cloning bounded,
    // but allow small terminal blocks to cover the common 5-8 use cleanup
    // fan-in cases seen in the decomp corpus.
    constexpr unsigned kMaxClonedTailUses                = 8;
    ...
    bool
    SeqEndsWithTerminator(clang::ASTContext &ctx, const std::vector< clang::Stmt * > &stmts) {
        if (stmts.empty()) { return false; }
            ...
            for (size_t if_idx = 0; if_idx + 1 < body.size(); ++if_idx) {
                ...
                auto build_if_from_sequence =
                    [&](const std::vector< clang::Stmt * > &sequence, size_t label_idx,
                        clang::Stmt *&new_if, std::vector< clang::Stmt * > &remainder) -> bool {
                    ...
                    remainder.assign(
                        sequence.begin() + static_cast< ptrdiff_t >(block.end), sequence.end()
                    );
                    return true;
                };
                ...
                auto *next_compound = llvm::dyn_cast< clang::CompoundStmt >(body[if_idx + 1]);
    ...
    bool TryBuildTerminalEpilogueRewriteForIf(
        clang::ASTContext &ctx, clang::IfStmt *ifs, const std::vector< clang::Stmt * > &body,
        const std::unordered_map< clang::LabelDecl *, unsigned > &refs,
        ...
        if (then_ref_it == refs.end() || else_ref_it == refs.end() || then_ref_it->second != 1
            || else_ref_it->second != 1)
        {
            return false;
        }
        ...
        std::vector< clang::Stmt * > common_suffix(
            then_block.stmts.end() - static_cast< ptrdiff_t >(suffix), then_block.stmts.end()
        );
        if (!SeqEndsWithTerminator(ctx, common_suffix)) { return false; }

        std::vector< clang::Stmt * > then_prefix(
            then_block.stmts.begin(), then_block.stmts.end() - static_cast< ptrdiff_t >(suffix)
        );
        std::vector< clang::Stmt * > else_prefix(
            else_block.stmts.begin(), else_block.stmts.end() - static_cast< ptrdiff_t >(suffix)
        );
        ...
        auto *new_if           = clang::IfStmt::Create(
            ctx, loc, clang::IfStatementKind::Ordinary, nullptr, nullptr, ifs->getCond(), loc,
            loc, then_stmt, loc, else_stmt
        );
    ...
    clang::Stmt *ReplaceStmtPtr(
        ...
        if (auto *compound = llvm::dyn_cast< clang::CompoundStmt >(stmt)) {
            ...
            bool before = changed;
            for (clang::Stmt *child : compound->body()) {
                children.push_back(ReplaceStmtPtr(ctx, child, target, replacement, changed));
            }
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ...
            if (ifs->getElse()) {
                ifs->setElse(ReplaceStmtPtr(ctx, ifs->getElse(), target, replacement, changed));
            }
            return stmt;
        }
        return stmt;
    ...
    void EraseLabelBlockRanges(
        std::vector< clang::Stmt * > &body, const LocalLabelBlock &a, const LocalLabelBlock &b
    ) {
    ...
    clang::Stmt *SinkCommonTerminalEpilogues(
        ...
        while (changed) {
            ...
            for (size_t i = 0; i < body.size(); ++i) {
                ...
                bool replaced = false;
                body[i] =
                    ReplaceStmtPtr(ctx, body[i], rewrite.site, rewrite.replacement, replaced);
    ...
    clang::Stmt *InlineSingleRefTerminalLabelBlocks(
        ...
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(stmt)) {
            ws->setBody(InlineSingleRefTerminalLabelBlocks(ctx, ws->getBody(), refs, mutated));
            return ws;
        }
        if (auto *ds = llvm::dyn_cast< clang::DoStmt >(stmt)) {
            ds->setBody(InlineSingleRefTerminalLabelBlocks(ctx, ds->getBody(), refs, mutated));
            return ds;
        }
        if (auto *fs = llvm::dyn_cast< clang::ForStmt >(stmt)) {
            fs->setBody(InlineSingleRefTerminalLabelBlocks(ctx, fs->getBody(), refs, mutated));
            return fs;
        }
        if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            ls->setSubStmt(
        ...
        if (auto *sw = llvm::dyn_cast< clang::SwitchStmt >(stmt)) {
            sw->setBody(InlineSingleRefTerminalLabelBlocks(ctx, sw->getBody(), refs, mutated));
            return sw;
        }
        ...
        while (changed) {
            ...
            for (size_t label_idx = 0; label_idx < body.size(); ++label_idx) {
                ...
                if (block_gotos == 0) {
                    ...
                    block_gotos == 1 && !GotoElimGetTarget(DeepTrailingStmt(block_stmt)).empty()
    ...
    clang::Stmt *
    CloneGotoFreeSeqAsStmt(clang::ASTContext &ctx, const std::vector< clang::Stmt * > &stmts) {
        std::vector< clang::Stmt * > cloned;
        for (clang::Stmt *stmt : stmts) {
    ...
    clang::Stmt *ReplaceGotoWithClonedJoinTail(
        clang::ASTContext &ctx, clang::Stmt *stmt, clang::LabelDecl *target,
        clang::LabelDecl *join, const std::vector< clang::Stmt * > &tail, unsigned &replaced,
        bool &failed
    ) {
    ...
    clang::Stmt *ReplaceGotoWithClonedGotoFreeSeq(
        clang::ASTContext &ctx, clang::Stmt *stmt, clang::LabelDecl *target,
        const std::vector< clang::Stmt * > &replacement_stmts, unsigned &replaced, bool &failed
    ) {
        if (!stmt || failed) { return stmt; }
        if (auto *gs = llvm::dyn_cast< clang::GotoStmt >(stmt)) {
            if (gs->getLabel() != target) { return stmt; }
    ...
    struct FallthroughTerminalJoinClone
    {
        clang::LabelDecl *target = nullptr;
        clang::LabelDecl *join   = nullptr;
        std::vector< clang::Stmt * > tail;
    };

    bool FindNestedFallthroughTerminalJoinClone(
        clang::ASTContext &ctx, clang::Stmt *stmt,
        const std::unordered_map< clang::LabelDecl *, unsigned > &refs,
        FallthroughTerminalJoinClone &clone
    );

    clang::Stmt *CloneSmallCrossScopeGotoTargets(
        clang::ASTContext &ctx, clang::Stmt *stmt,
    ...
    clang::Stmt *CloneFallthroughTerminalLabelGotos(
        ...
        for (size_t label_idx = 0; label_idx < body.size(); ++label_idx) {
            ...
            bool failed                = false;
            clang::Stmt *block_stmt    = StmtFromSeq(ctx, block.stmts);
            const unsigned block_gotos = CountAllGotos(block_stmt);
            if (block_gotos == 0) {
                if (SeqHasUnsafeStructure(
                        block.stmts, /*allow_goto=*/false, /*allow_return=*/true
                    ))
            ...
            if (block_gotos == 1 && !block.stmts.empty()) {
                if (ref_it->second > kMaxClonedTailUses) { continue; }
                auto *join_goto = llvm::dyn_cast< clang::GotoStmt >(block.stmts.back());
                if (!join_goto || !join_goto->getLabel() || join_goto->getLabel() == target) {
                    continue;
                }

                std::vector< clang::Stmt * > tail(block.stmts.begin(), block.stmts.end() - 1);
                if (!StraightLineContinuationSeqIsSafe(tail)) { continue; }

                for (size_t i = 0; i < body.size(); ++i) {
                    if (i >= block.begin && i < block.end) { continue; }
                    body[i] = ReplaceGotoWithClonedJoinTail(
                        ctx, body[i], target, join_goto->getLabel(), tail, replaced, failed
                    );
                    if (failed) { break; }
                }
                if (!failed && replaced == ref_it->second) {
        ...
        bool copied_cross_scope = false;
        auto *rebuilt           = detail::MakeCompound(ctx, body);
        return CloneSmallCrossScopeGotoTargets(ctx, rebuilt, refs, copied_cross_scope);
    }
    ...
    bool FindNestedFallthroughTerminalJoinClone(
        ...
        if (auto *compound = llvm::dyn_cast< clang::CompoundStmt >(stmt)) {
            ...
            for (size_t label_idx = 0; label_idx < body.size(); ++label_idx) {
                ...
                if (!SeqEndsWithTerminator(ctx, block.stmts)) { continue; }
                if (CountAllGotos(StmtFromSeq(ctx, block.stmts)) != 1) { continue; }
                ...
                clone.target = target;
                clone.join   = join_goto->getLabel();
                clone.tail   = std::move(tail);
                return true;
            }
        }

        for (clang::Stmt *child : stmt->children()) {
            if (FindNestedFallthroughTerminalJoinClone(ctx, child, refs, clone)) {
                return true;
            }
        }
        return false;
    ...
    clang::Stmt *CloneSmallCrossScopeGotoTargets(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ifs->setThen(CloneSmallCrossScopeGotoTargets(ctx, ifs->getThen(), refs, changed));
            if (ifs->getElse()) {
                ifs->setElse(
                    CloneSmallCrossScopeGotoTargets(ctx, ifs->getElse(), refs, changed)
                );
            }
            return ifs;
        }
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(stmt)) {
            ws->setBody(CloneSmallCrossScopeGotoTargets(ctx, ws->getBody(), refs, changed));
            return ws;
        }
        if (auto *ds = llvm::dyn_cast< clang::DoStmt >(stmt)) {
            ds->setBody(CloneSmallCrossScopeGotoTargets(ctx, ds->getBody(), refs, changed));
            return ds;
        }
        if (auto *fs = llvm::dyn_cast< clang::ForStmt >(stmt)) {
            fs->setBody(CloneSmallCrossScopeGotoTargets(ctx, fs->getBody(), refs, changed));
            return fs;
        }
        if (auto *label = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            label->setSubStmt(
                CloneSmallCrossScopeGotoTargets(ctx, label->getSubStmt(), refs, changed)
            );
            return label;
        }
        if (auto *sw = llvm::dyn_cast< clang::SwitchStmt >(stmt)) {
            sw->setBody(CloneSmallCrossScopeGotoTargets(ctx, sw->getBody(), refs, changed));
            return sw;
        }
        ...
        for (clang::Stmt *&child : body) {
            ...
            child = CloneSmallCrossScopeGotoTargets(ctx, child, refs, child_changed);
            if (child_changed) { body_changed = true; }
        }
        if (body_changed) {
            changed = true;
            return detail::MakeCompound(ctx, body);
        ...
        for (size_t child_idx = 0; child_idx < body.size(); ++child_idx) {
            FallthroughTerminalJoinClone clone;
            if (!FindNestedFallthroughTerminalJoinClone(ctx, body[child_idx], refs, clone)) {
                continue;
            }
            ...
            for (size_t i = 0; i < body.size(); ++i) {
                ...
                body[i] = ReplaceGotoWithClonedJoinTail(
                    ctx, body[i], clone.target, clone.join, clone.tail, replaced, failed
                );
                if (failed) { break; }
            }
            if (failed || replaced == 0) { continue; }
    ...
    clang::Stmt *CloneNoFallthroughTerminalLabelGotos(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ...
            if (ifs->getElse()) {
                ifs->setElse(CloneNoFallthroughTerminalLabelGotos(ctx, ifs->getElse(), refs));
            }
            return ifs;
        }
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(stmt)) {
    ...
    bool SeqHasUnsafeGuardedJoinMoveControl(const std::vector< clang::Stmt * > &stmts) {
        for (clang::Stmt *stmt : stmts) {
    ...
    clang::Stmt *ReplaceGotoWithClonedCleanupThenJoin(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ifs->setThen(ReplaceGotoWithClonedCleanupThenJoin(
                ctx, ifs->getThen(), cleanup_label, join_label, cleanup_stmts, replaced, failed
            ));
            if (failed) { return stmt; }
            if (ifs->getElse()) {
    ...
    clang::Stmt *CloneCleanupLabelBeforeJoinGotos(
        ...
        while (local_changed) {
            ...
            for (size_t label_idx = 0; label_idx + 1 < body.size(); ++label_idx) {
                ...
                if (!ExtractLocalLabelBlock(ctx, body, label_idx, cleanup_block)) { continue; }
                ...
                for (size_t i = 0; i < cleanup_block.begin; ++i) {
                    body[i] = ReplaceGotoWithClonedCleanupThenJoin(
                        ctx, body[i], cleanup_label, join_label, cleanup_block.stmts, replaced,
                        failed
                    );
                    if (failed) { break; }
                }
    ...
    clang::Stmt *CloneSmallStraightLineLabelBeforeJoinGotos(
        ...
        if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            ls->setSubStmt(
                CloneSmallStraightLineLabelBeforeJoinGotos(ctx, ls->getSubStmt(), refs, changed)
            );
            return ls;
        }
        ...
        while (local_changed) {
            ...
            for (size_t label_idx = 0; label_idx + 1 < body.size(); ++label_idx) {
                ...
                if (!ExtractLocalLabelBlock(ctx, body, label_idx, target_block)) { continue; }
                ...
                for (size_t i = 0; i < target_block.begin; ++i) {
                    body[i] = ReplaceGotoWithClonedCleanupThenJoin(
                        ctx, body[i], target_label, join_label, target_block.stmts, replaced,
                        failed
                    );
                    if (failed) { break; }
                }
    ...
    bool TryFoldThreeGuardJoinAt(
        ...
        auto *normal_if = clang::IfStmt::Create(
            ctx, join_guard->getIfLoc(), clang::IfStatementKind::Ordinary, nullptr, nullptr,
            NegateExpr(ctx, join_guard->getCond()), join_guard->getLParenLoc(),
            join_guard->getRParenLoc(), StmtFromSeq(ctx, normal_region)
        );
        auto *alt_if = clang::IfStmt::Create(
            ctx, alt_guard->getIfLoc(), clang::IfStatementKind::Ordinary, nullptr, nullptr,
            alt_guard->getCond(), alt_guard->getLParenLoc(), alt_guard->getRParenLoc(),
            StmtFromSeq(ctx, alt_block.stmts), alt_guard->getElseLoc(), normal_if
        );
    ...
    clang::Stmt *FoldInternalGotoToJoin(
        clang::ASTContext &ctx, clang::Stmt *stmt, clang::LabelDecl *join_label, bool &changed
    );

    bool FoldInternalGotoToJoinInSeq(
        clang::ASTContext &ctx, std::vector< clang::Stmt * > &body,
        clang::LabelDecl *join_label, bool &changed
    ) {
        if (!join_label) { return false; }

        for (clang::Stmt *&child : body) {
            child = FoldInternalGotoToJoin(ctx, child, join_label, changed);
        }
        ...
        for (size_t i = 0; i + 1 < body.size(); ++i) {
            ...
            AppendStmtSequence(then_stmt, then_body);
            if (then_body.empty() || SeqHasUnsafeGuardedJoinMoveControl(then_body)) {
                continue;
            }
            ...
            if (else_body.empty() || SeqHasUnsafeGuardedJoinMoveControl(else_body)) {
                continue;
            }

            auto loc     = ifs->getIfLoc();
            auto *folded = clang::IfStmt::Create(
                ctx, loc, clang::IfStatementKind::Ordinary, nullptr, nullptr,
                CloneExpr(ctx, ifs->getCond()), ifs->getLParenLoc(), ifs->getRParenLoc(),
                StmtFromSeq(ctx, then_body), ifs->getElseLoc(), StmtFromSeq(ctx, else_body)
            );

            body.erase(body.begin() + static_cast< ptrdiff_t >(i), body.end());
            body.push_back(folded);
            changed = true;
            return true;
        }
    ...
    clang::Stmt *FoldInternalGotoToJoin(
        ...
        if (!stmt || !join_label) { return stmt; }

        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ifs->setThen(FoldInternalGotoToJoin(ctx, ifs->getThen(), join_label, changed));
            if (ifs->getElse()) {
                ifs->setElse(FoldInternalGotoToJoin(ctx, ifs->getElse(), join_label, changed));
            }
            return ifs;
        }
        if (auto *compound = llvm::dyn_cast< clang::CompoundStmt >(stmt)) {
            ...
            while (FoldInternalGotoToJoinInSeq(ctx, body, join_label, local_changed)) {}
            if (local_changed) {
                changed = true;
                return StmtFromSeq(ctx, body);
            }
            return stmt;
        }
        return stmt;
    ...
    bool TryFoldIfGotoAltThenJoinAt(
        ...
        LocalLabelBlock alt_block;
        if (!FindLabelBlockByDecl(ctx, body, alt_label, alt_block)) { return false; }
        if (alt_block.begin <= if_idx + 1 || alt_block.end >= body.size()) { return false; }
        ...
        bool folded_internal    = false;
        clang::Stmt *false_stmt = FoldInternalGotoToJoin(
            ctx, StmtFromSeq(ctx, false_stripped), join_label, folded_internal
        );
        false_stripped.clear();
        AppendStmtSequence(false_stmt, false_stripped);
        if (SeqHasUnsafeGuardedJoinMoveControl(false_stripped)) { return false; }
    ...
    bool TryFoldIfThenJoinElseAltAt(
        ...
        // This fold drops the alt label, so only proceed when nothing jumps to
        // it (refs is name-coalesced; a non-zero count would strand a goto).
        ...
        clang::Stmt *stripped_then =
            StripTrailingGoto(ctx, ifs->getThen(), join_label->getName());
        bool folded_internal = false;
        stripped_then = FoldInternalGotoToJoin(ctx, stripped_then, join_label, folded_internal);
        then_stripped.clear();
    ...
    clang::Stmt *FoldGuardedJoinLabelChains(
        ...
        if (!stmt) { return stmt; }
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ifs->setThen(FoldGuardedJoinLabelChains(ctx, ifs->getThen(), refs, changed));
            if (ifs->getElse()) {
                ifs->setElse(FoldGuardedJoinLabelChains(ctx, ifs->getElse(), refs, changed));
            }
            return ifs;
        }
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(stmt)) {
        ...
        if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            ls->setSubStmt(FoldGuardedJoinLabelChains(ctx, ls->getSubStmt(), refs, changed));
            return ls;
        }
        if (llvm::isa< clang::SwitchStmt >(stmt) || llvm::isa< clang::CaseStmt >(stmt)
            || llvm::isa< clang::DefaultStmt >(stmt))
    ...
    clang::Stmt *FoldCrossCompoundIfLabelDiamonds(
        ...
        for (size_t if_idx = 0; if_idx + 2 < body.size(); ++if_idx) {
            ...
            // refs goes stale as this loop folds; re-count gotos to the alt
            // label in the current body and bail unless all are the ones folded
            // here (else a surviving same-named goto is stranded).
            {
                unsigned live = 0;
            ...
            // The fold also drops the join label; join_ref > join_gotos means a
            // goto outside this `then` still targets it, so skip (would orphan).
            auto join_ref = refs.find(join_label);
            ...
            clang::Stmt *then_rewritten = ReplaceGotoWithClonedJoinTail(
                ctx, ifs->getThen(), alt_label, join_label, alt_block.stmts, replaced, failed
            );
            ...
            auto *new_if = clang::IfStmt::Create(
                ctx, loc, clang::IfStatementKind::Ordinary, nullptr, nullptr, ifs->getCond(),
                loc, loc, then_rewritten, loc, else_stmt
            );
            ...
            // Post-fold safety: if the folded body references a dropped label
            // name with no surviving LabelStmt, revert (stmt is intact) rather
            // than emit an orphaned goto.
    ...
    bool TryFoldCompoundDispatchChainAt(
        ...
        auto try_prefix = [&](size_t guard_count) -> bool {
            ...
            for (const auto &[label, count] : guard_refs) {
                ...
                if (ref_it == refs.end() || ref_it->second != count) { return false; }
            }

            size_t default_start = guard_idx + guard_count;
            if (default_start >= body.size()) { return false; }
            ...
            for (const DispatchGuard &guard : guards) {
                ...
                LocalLabelBlock block;
                if (!FindLabelBlockByDecl(ctx, body, guard.target, block)) { return false; }
                if (block.begin <= default_start) { return false; }
            ...
            if (first_label_idx == body.size() || join_idx >= body.size()) { return false; }

            for (size_t i = default_start; i < first_label_idx; ++i) {
                if (LeadingLabelDecl(body[i])) { return false; }
            }
            ...
            for (const DispatchGuard &guard : guards) {
                ...
                if (stmts.empty() || stmts.size() > kMaxTerminalPrefixStmts) { return false; }
                ...
                if (SeqHasUnsafeStructure(stmts, /*allow_goto=*/false, /*allow_return=*/false))
                {
                    return false;
                }
    ...
    bool TryFoldCrossCompoundDispatchArmAt(
        clang::ASTContext &ctx, std::vector< clang::Stmt * > &parent_body, size_t carrier_idx,
        clang::Stmt *&arm, const std::unordered_map< clang::LabelDecl *, unsigned > &refs
    ) {
        auto *compound = llvm::dyn_cast_or_null< clang::CompoundStmt >(arm);
        if (!compound || carrier_idx + 1 >= parent_body.size()) { return false; }

        std::vector< clang::Stmt * > arm_body(compound->body_begin(), compound->body_end());
        for (size_t guard_idx = 0; guard_idx + 1 < arm_body.size(); ++guard_idx) {
            ...
            auto try_prefix = [&](size_t guard_count) -> bool {
                ...
                std::vector< clang::Stmt * > default_region(
                    arm_body.begin() + static_cast< ptrdiff_t >(default_start), arm_body.end()
                );
                if (default_region.empty()) { return false; }
                if (SeqHasUnsafeStructure(
                        default_region, /*allow_goto=*/true, /*allow_return=*/true
    ...
    clang::Stmt *GetLoopBody(clang::Stmt *stmt) {
        ...
        if (auto *ds = llvm::dyn_cast_or_null< clang::DoStmt >(stmt)) { return ds->getBody(); }
        if (auto *fs = llvm::dyn_cast_or_null< clang::ForStmt >(stmt)) { return fs->getBody(); }
        return nullptr;
    }
    ...
    clang::Stmt *ReplaceGotoWithBreakInCurrentLoop(
        clang::ASTContext &ctx, clang::Stmt *stmt, clang::LabelDecl *target, unsigned &replaced
    ) {
        if (!stmt) { return stmt; }
        if (auto *gs = llvm::dyn_cast< clang::GotoStmt >(stmt)) {
            if (gs->getLabel() == target) {
        ...
        if (auto *label = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            label->setSubStmt(
                ReplaceGotoWithBreakInCurrentLoop(ctx, label->getSubStmt(), target, replaced)
            );
            return stmt;
        }
        return stmt;
    ...
    clang::Stmt *ConvertImmediateLoopExitGotosToBreak(
        ...
        if (auto *default_stmt = llvm::dyn_cast< clang::DefaultStmt >(stmt)) {
            default_stmt->setSubStmt(
                ConvertImmediateLoopExitGotosToBreak(ctx, default_stmt->getSubStmt(), mutated)
            );
            return default_stmt;
        }
    ...
    clang::Stmt *PromoteLocalBackwardGotoLoops(
        ...
        if (auto *label = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            label->setSubStmt(PromoteLocalBackwardGotoLoops(ctx, label->getSubStmt(), refs));
            return label;
        }
        if (auto *sw = llvm::dyn_cast< clang::SwitchStmt >(stmt)) {
        ...
        while (changed) {
            ...
            for (size_t label_idx = 0; label_idx < body.size(); ++label_idx) {
                ...
                clang::Stmt *loop_body =
                    StripTrailingGoto(ctx, StmtFromSeq(ctx, block.stmts), target->getName());
                ...
                body.insert(body.begin() + static_cast< ptrdiff_t >(block.begin), while_stmt);
                changed = true;
                break;
            }
        }
    ...
    bool
    SeqHasLocalLabelOrControl(const std::vector< clang::Stmt * > &stmts, bool allow_return) {
    ...
    clang::Stmt *FoldLocalGotoDiamonds(
        ...
        std::vector< clang::Stmt * > body(compound->body_begin(), compound->body_end());
        // Recompute refs over the current body before each child so gotos a
        // prior sibling's fold created are counted (else a stale count strands them).
            ...
            for (size_t if_idx = 0; if_idx + 2 < body.size(); ++if_idx) {
                ...
                if (ifs->getElse()) {
                    auto try_fold_if_else_goto_arm = [&](bool goto_is_then) -> bool {
                        clang::Stmt *goto_arm = goto_is_then ? ifs->getThen() : ifs->getElse();
                        clang::Stmt *body_arm = goto_is_then ? ifs->getElse() : ifs->getThen();
                        ...
                        // refs goes stale as this loop folds; this fold erases
                        // alt_label, so re-count gotos to it by name now and bail
                        // unless it is still the sole reference (else orphaned).
                        ...
                        auto loc              = ifs->getIfLoc();
                        clang::Stmt *new_then = goto_is_then ? StmtFromSeq(ctx, alt_region)
                        ...
                        // Post-fold safety: decline the fold if the tentative
                        // result strands any goto (target name with no LabelStmt).
                        {
                ...
                if (then_label_idx >= body.size() || then_label_idx == if_idx + 1) { continue; }
                ...
                // Decline the fold if the result strands any goto (the erased
                // range may hold intermediate labels beyond then/join).
                {
                    std::unordered_set< std::string > labels, gotos;
                    std::function< void(clang::Stmt *) > w = [&](clang::Stmt *st) {
                        if (!st) { return; }
    ...
    void
    CollectDefinedLabels(clang::Stmt *s, std::unordered_set< clang::LabelDecl * > &defined) {
    ...
    clang::Stmt *RemoveOrphanedGotos(
        ...
        if (auto *cs = llvm::dyn_cast< clang::CompoundStmt >(s)) {
            ...
            bool changed = false;
            for (auto *child : cs->body()) {
                auto *repl = RemoveOrphanedGotos(ctx, child, defined, depth + 1, child_in_case);
    ...
    bool AnalyzeSharedLatchChildren(
        clang::ASTContext &ctx, clang::CompoundStmt *compound, const clang::VarDecl *counter,
        ...
        for (size_t i = 0; i < children.size(); ++i) {
            ...
            if (auto *nested = llvm::dyn_cast< clang::CompoundStmt >(children[i])) {
                if (!AnalyzeSharedLatchChildren(ctx, nested, counter, plan)) { return false; }
                continue;
            }
    ...
    clang::Stmt *RemoveSharedLatchIncrements(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ifs->setThen(RemoveSharedLatchIncrements(ctx, ifs->getThen(), counter, inc_text));
            if (ifs->getElse()) {
                ifs->setElse(
    ...
    bool TryPromoteWhileAt(
        ...
        if (!while_stmt || !while_stmt->getCond() || !while_stmt->getBody()) { return false; }
        ...
        for (size_t i = 0; i < loop_body.size(); ++i) {
            ...
            if (HasUnsafeForPromotionControl(loop_body[i], terminal_continue)) {
                LatchRewritePlan plan;
                if (!AnalyzeSharedLatchInStmt(ctx, while_stmt->getBody(), counter, plan, false)
                    || !plan.inc || plan.continues == 0 || plan.continues != plan.latch_writes
    ...
    clang::Stmt *
    PromoteSimpleCounterWhileToFor(clang::ASTContext &ctx, clang::Stmt *stmt, bool &mutated) {
        if (!stmt) { return stmt; }
    ...
    clang::Stmt *RemoveRedundantTerminalForContinues(
        ...
        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ...
            if (ifs->getElse()) {
                ifs->setElse(RemoveRedundantTerminalForContinues(ctx, ifs->getElse(), mutated));
            }
            return ifs;
        }
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(stmt)) {
        ...
        if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            ls->setSubStmt(RemoveRedundantTerminalForContinues(ctx, ls->getSubStmt(), mutated));
            return ls;
        }
        if (auto *sw = llvm::dyn_cast< clang::SwitchStmt >(stmt)) {
        ...
        if (auto *default_stmt = llvm::dyn_cast< clang::DefaultStmt >(stmt)) {
            default_stmt->setSubStmt(
                RemoveRedundantTerminalForContinues(ctx, default_stmt->getSubStmt(), mutated)
            );
            return default_stmt;
        }
        if (auto *for_stmt = llvm::dyn_cast< clang::ForStmt >(stmt)) {
        ...
        if (auto *compound = llvm::dyn_cast< clang::CompoundStmt >(stmt)) {
            ...
            for (clang::Stmt *child : compound->body()) {
                children.push_back(RemoveRedundantTerminalForContinues(ctx, child, mutated));
            }
            return detail::MakeCompound(ctx, children);
        }
        return stmt;
    ...
    clang::Stmt *
    PushLabelsIntoCompounds(clang::ASTContext &ctx, clang::Stmt *stmt, bool &mutated) {
        if (!stmt) { return stmt; }
        ...
        if (auto *case_stmt = llvm::dyn_cast< clang::CaseStmt >(stmt)) {
            ...
            if (!compound) {
                case_stmt->setSubStmt(sub ? sub : new (ctx) clang::NullStmt(VirtualLoc(ctx)));
                return case_stmt;
            }
            mutated = true;
        ...
        if (auto *default_stmt = llvm::dyn_cast< clang::DefaultStmt >(stmt)) {
            auto *sub      = PushLabelsIntoCompounds(ctx, default_stmt->getSubStmt(), mutated);
            auto *compound = llvm::dyn_cast_or_null< clang::CompoundStmt >(sub);
            if (!compound) {
                default_stmt->setSubStmt(
    ...
    clang::Stmt *
    AttachEmptyLabelsToFollowingStmt(clang::ASTContext &ctx, clang::Stmt *stmt, bool &mutated) {
        if (!stmt) { return stmt; }

        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(stmt)) {
            ifs->setThen(AttachEmptyLabelsToFollowingStmt(ctx, ifs->getThen(), mutated));
            if (ifs->getElse()) {
                ifs->setElse(AttachEmptyLabelsToFollowingStmt(ctx, ifs->getElse(), mutated));
            }
            return ifs;
        }
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(stmt)) {
        ...
        if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
            ls->setSubStmt(AttachEmptyLabelsToFollowingStmt(ctx, ls->getSubStmt(), mutated));
            return ls;
        }
        if (auto *sw = llvm::dyn_cast< clang::SwitchStmt >(stmt)) {
        ...
        if (auto *default_stmt = llvm::dyn_cast< clang::DefaultStmt >(stmt)) {
            default_stmt->setSubStmt(
                AttachEmptyLabelsToFollowingStmt(ctx, default_stmt->getSubStmt(), mutated)
            );
            return default_stmt;
        }
        ...
        for (size_t i = 0; i + 1 < children.size(); ++i) {
            ...
            if (!next || llvm::isa< clang::DeclStmt >(next)
                || llvm::isa< clang::CaseStmt >(next) || llvm::isa< clang::DefaultStmt >(next))
            {
                continue;
            }
    ...
    clang::Expr *
    SimplifyRedundantNonzeroGuard(clang::ASTContext &ctx, clang::Expr *lhs, clang::Expr *rhs) {
    ...
    clang::Expr *NormalizeBoolExpr(clang::ASTContext &ctx, clang::Expr *e) {
        ...
        if (auto *bo = llvm::dyn_cast< clang::BinaryOperator >(e)) {
            if (bo->getOpcode() == clang::BO_And) {
                auto build_bitmask_compare = [&](clang::Expr *value,
                    ...
                    auto *masked = clang::BinaryOperator::Create(
                        ctx, EnsureRValue(ctx, value), EnsureRValue(ctx, mask), clang::BO_And,
                        value->getType(), clang::VK_PRValue, clang::OK_Ordinary, loc,
                        clang::FPOptionsOverride()
                    );
                    auto *paren_masked = new (ctx) clang::ParenExpr(loc, loc, masked);
                    return clang::BinaryOperator::Create(
                        ctx, paren_masked, EnsureRValue(ctx, rhs), cmp->getOpcode(), ctx.BoolTy,
            ...
            if (bo->isEqualityOp() || bo->isRelationalOp()) {
                auto parenthesize_bitwise = [&](clang::Expr *expr) -> clang::Expr * {
                    auto *child =
                        llvm::dyn_cast_or_null< clang::BinaryOperator >(expr->IgnoreParens());
                    ...
                    return new (ctx) clang::ParenExpr(VirtualLoc(ctx), VirtualLoc(ctx), expr);
                };
        ...
        if (auto *inner_logic = llvm::dyn_cast< clang::BinaryOperator >(sub)) {
            ...
            {
                auto flipped =
                    inner_logic->getOpcode() == clang::BO_LAnd ? clang::BO_LOr : clang::BO_LAnd;
                ...
                return clang::BinaryOperator::Create(
                    ...
                    flipped, ctx.BoolTy, clang::VK_PRValue, clang::OK_Ordinary, VirtualLoc(ctx),
                    clang::FPOptionsOverride()
                );
            }
        }
        ...
        if (auto *inner_bo = llvm::dyn_cast< clang::BinaryOperator >(sub)) {
            ...
            if (can_flip) {
                return clang::BinaryOperator::Create(
                    ctx, inner_bo->getLHS(), inner_bo->getRHS(), flipped, inner_bo->getType(),
                    inner_bo->getValueKind(), inner_bo->getObjectKind(),
                    inner_bo->getOperatorLoc(), clang::FPOptionsOverride()
                );
            }
        }
    ...
    // Collapse clone-induced duplicate same-named labels: re-point every goto
    // onto the canonical (first) same-named LabelStmt and unwrap non-canonical
    // duplicates (keeping their body), leaving one LabelStmt per name so
    // decl-keyed liveness can't desync and strand a goto. No-op when names are
    // unique.
    ...
    RemoveDeadControlFlow(clang::ASTContext &ctx, clang::Stmt *body, bool &mutated) {
        ...
        // Collapse duplicate same-named labels before any decl-keyed liveness
        // check, so a surviving label isn't mistaken for dead and its goto stranded.
        ...
        if (auto *repaired = RemoveOrphanedGotos(ctx, body, defined)) { body = repaired; }
    ...
    clang::Stmt *HoistCrossScopeLabels(
        ...
        const unsigned pre_repair_gotos = CountAllGotos(body);
        std::unordered_set< clang::LabelDecl * > pre_repair_labels;
        CollectDefinedLabels(body, pre_repair_labels);
        ...
        // 2. Hoist cross-scope label entries to the enclosing compound only
        // for large or label-dense irreducible bodies.  The hoist is a
        // correctness-preserving normalization, but it pays an upfront tax:
        // every extracted label needs a synthetic skip-to-join goto.  For
        // compact functions, the repaired nested-label form is usually clearer
        // and avoids introducing synthetic joins that later passes may not
        // fold.  Label-dense bodies still benefit from hoisting because it
        // exposes downstream label-region folds even when the raw goto count
        // is moderate.
        constexpr unsigned kMinGotosForCrossScopeHoist  = 20;
        constexpr unsigned kMinLabelsForCrossScopeHoist = 80;
        if (pre_repair_gotos >= kMinGotosForCrossScopeHoist
            || pre_repair_labels.size() >= kMinLabelsForCrossScopeHoist)
        {
            bool hoisted = false;
    ...
    clang::Stmt *FoldSmallCrossScopeGotoTargets(
        ...
        constexpr unsigned kMaxSmallCrossScopeHoistGotos = 5;
        std::unordered_map< clang::LabelDecl *, unsigned > refs;
        CountGotoDeclRefs(body, refs);

        if (CountAllGotos(body) <= kMaxSmallCrossScopeHoistGotos) {
            FallthroughTerminalJoinClone clone;
            if (FindNestedFallthroughTerminalJoinClone(ctx, body, refs, clone)) {
                bool hoisted = false;
                body         = HoistCrossScopeLabelEntries(ctx, fn, body, hoisted);
                if (hoisted) {
                    refs.clear();
        ...
        bool copied = false;
        body        = CloneSmallCrossScopeGotoTargets(ctx, body, refs, copied);
    ...
    // Undo HoistCrossScopeLabelEntries' synthetic join: fold
    //   if(A1) {...; goto J;} if(A2) {...} J:;
    // into
    //   if(A1) {...} else if(A2) {...}
    // Only touches labels named `__patchestry_scope_join` whose
    // sub-stmt is empty. If J has refs outside the chain, the chain
    // still folds but the label is kept.
    namespace {

        clang::LabelDecl *TrailingGotoLabel(clang::Stmt *stmt) {
            if (!stmt) { return nullptr; }
            if (auto *gs = llvm::dyn_cast< clang::GotoStmt >(stmt)) { return gs->getLabel(); }
            if (auto *cs = llvm::dyn_cast< clang::CompoundStmt >(stmt)) {
                if (cs->body_empty()) { return nullptr; }
                return TrailingGotoLabel(cs->body_back());
            }
            if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(stmt)) {
                return TrailingGotoLabel(ls->getSubStmt());
            }
            return nullptr;
        }

        bool IsScopeJoinLabel(clang::LabelDecl *decl) {
            return decl && decl->getName().starts_with("__patchestry_scope_join");
        }

        bool LabelSubStmtIsEmpty(clang::LabelStmt *ls) {
            auto *sub = ls->getSubStmt();
            if (!sub) { return true; }
            if (llvm::isa< clang::NullStmt >(sub)) { return true; }
            if (auto *cs = llvm::dyn_cast< clang::CompoundStmt >(sub)) {
        ...
        bool
        FoldScopeJoinChainOnce(clang::ASTContext &ctx, std::vector< clang::Stmt * > &stmts) {
            std::unordered_map< clang::LabelDecl *, unsigned > refs;
            auto *probe = detail::MakeCompound(ctx, stmts);
            CountGotoDeclRefs(probe, refs);

            for (size_t i = 0; i + 1 < stmts.size(); ++i) {
                auto *first_if = llvm::dyn_cast< clang::IfStmt >(stmts[i]);
                if (!first_if || first_if->getElse()) { continue; }

                clang::LabelDecl *J = TrailingGotoLabel(first_if->getThen());
                if (!IsScopeJoinLabel(J)) { continue; }

                // Collect contiguous if-without-else stmts. Each either
                // ends in `goto J` or falls through (last element only).
                ...
                size_t k = i + 1;
                while (k < stmts.size()) {
                    auto *cur = llvm::dyn_cast< clang::IfStmt >(stmts[k]);
                    if (!cur || cur->getElse()) { break; }
                    clang::LabelDecl *tg = TrailingGotoLabel(cur->getThen());
                    if (tg == J) {
                        chain.push_back(k);
                        term.push_back(true);
                        ++k;
                ...
                // Next sibling must be J's empty LabelStmt.
                if (k >= stmts.size()) { continue; }
                auto *lbl = llvm::dyn_cast< clang::LabelStmt >(stmts[k]);
                if (!lbl || lbl->getDecl() != J) { continue; }
                if (!LabelSubStmtIsEmpty(lbl)) { continue; }

                unsigned chain_refs = 0;
                for (bool t : term) {
                    if (t) { ++chain_refs; }
                }
                unsigned total_refs = 0;
                if (auto it = refs.find(J); it != refs.end()) { total_refs = it->second; }
                bool drop_label = (chain_refs >= total_refs);

                // Back-to-front build; fall-through tail becomes the
                // innermost else.
                clang::Stmt *else_branch = nullptr;
                size_t fold_count        = chain.size();
                if (!term.back()) {
                    else_branch = stmts[chain.back()];
                    --fold_count;
                }
                for (ptrdiff_t idx = static_cast< ptrdiff_t >(fold_count) - 1; idx >= 0; --idx)
                {
                    auto *cur =
                        llvm::cast< clang::IfStmt >(stmts[chain[static_cast< size_t >(idx)]]);
                    clang::Stmt *then_body =
                        StripTrailingGoto(ctx, cur->getThen(), J->getName());
                    auto loc    = cur->getIfLoc();
                    else_branch = clang::IfStmt::Create(
                        ctx, loc, clang::IfStatementKind::Ordinary, nullptr, nullptr,
                        CloneExpr(ctx, cur->getCond()), loc, loc, then_body, loc, else_branch
                    );
                }

                size_t erase_end = drop_label ? (k + 1) : k;
                stmts.erase(
                    stmts.begin() + static_cast< ptrdiff_t >(i),
                    stmts.begin() + static_cast< ptrdiff_t >(erase_end)
                );
                stmts.insert(stmts.begin() + static_cast< ptrdiff_t >(i), else_branch);
                return true;
            }
            return false;
        }
    ...
    clang::Stmt *
    FoldScopeJoinIfElseChain(clang::ASTContext &ctx, clang::Stmt *body, bool &mutated) {
        if (!body) { return body; }

        if (auto *ifs = llvm::dyn_cast< clang::IfStmt >(body)) {
            ifs->setThen(FoldScopeJoinIfElseChain(ctx, ifs->getThen(), mutated));
            if (ifs->getElse()) {
                ifs->setElse(FoldScopeJoinIfElseChain(ctx, ifs->getElse(), mutated));
            }
            return ifs;
        }
        if (auto *ws = llvm::dyn_cast< clang::WhileStmt >(body)) {
            ws->setBody(FoldScopeJoinIfElseChain(ctx, ws->getBody(), mutated));
            return ws;
        }
        if (auto *ds = llvm::dyn_cast< clang::DoStmt >(body)) {
            ds->setBody(FoldScopeJoinIfElseChain(ctx, ds->getBody(), mutated));
            return ds;
        }
        if (auto *fs = llvm::dyn_cast< clang::ForStmt >(body)) {
            fs->setBody(FoldScopeJoinIfElseChain(ctx, fs->getBody(), mutated));
            return fs;
        }
        if (auto *sw = llvm::dyn_cast< clang::SwitchStmt >(body)) {
            sw->setBody(FoldScopeJoinIfElseChain(ctx, sw->getBody(), mutated));
            return sw;
        }
        if (auto *ls = llvm::dyn_cast< clang::LabelStmt >(body)) {
            ls->setSubStmt(FoldScopeJoinIfElseChain(ctx, ls->getSubStmt(), mutated));
            return ls;
        }
        if (auto *cs = llvm::dyn_cast< clang::CaseStmt >(body)) {
            cs->setSubStmt(FoldScopeJoinIfElseChain(ctx, cs->getSubStmt(), mutated));
            return cs;
        }
        if (auto *ds = llvm::dyn_cast< clang::DefaultStmt >(body)) {
            ds->setSubStmt(FoldScopeJoinIfElseChain(ctx, ds->getSubStmt(), mutated));
            return ds;
        }

        auto *compound = llvm::dyn_cast< clang::CompoundStmt >(body);
        if (!compound) { return body; }

        std::vector< clang::Stmt * > stmts(compound->body_begin(), compound->body_end());
        for (clang::Stmt *&s : stmts) { s = FoldScopeJoinIfElseChain(ctx, s, mutated); }

        while (FoldScopeJoinChainOnce(ctx, stmts)) { mutated = true; }
