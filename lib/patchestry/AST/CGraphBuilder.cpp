...
namespace patchestry::ast {
    ...
    namespace {
        ...
        std::vector< std::string > compute_rpo_from_function(const ghidra::Function &function) {
            ...
            if (post_order.size() < function.basic_blocks.size()) {
                ...
                auto parse_key = [](const std::string &k) -> std::pair< uint64_t, uint64_t > {
                    ...
                    try {
                        addr = std::stoull(k.substr(p1 + 1, p2 - p1 - 1), nullptr, 16);
                    } catch (const std::invalid_argument &e) {
                        LOG(ERROR) << "malformed block key address in '" << k
                                   << "': " << e.what() << "\n";
                    } catch (const std::out_of_range &e) {
                        LOG(ERROR) << "block key address out of range in '" << k
                                   << "': " << e.what() << "\n";
                    }
                    if (p3 != std::string::npos) {
                        try {
                            idx = std::stoull(k.substr(p2 + 1, p3 - p2 - 1));
                        } catch (const std::invalid_argument &e) {
                            LOG(ERROR) << "malformed block key index in '" << k
                                       << "': " << e.what() << "\n";
                        } catch (const std::out_of_range &e) {
                            LOG(ERROR) << "block key index out of range in '" << k
                                       << "': " << e.what() << "\n";
                        }
                    }
                    return { addr, idx };
    ...
    CGraph BuildCGraph(FunctionBuilder &builder, clang::ASTContext &ctx) {
        ...
        for (size_t i = 0; i < rpo.size(); ++i) { key_to_index[rpo[i]] = i; }
        ...
        for (size_t i = 0; i < rpo.size(); ++i) {
            ...
            if (!term) {
                ...
                if (i + 1 < rpo.size()) {
                    add_source_edge(g, node, key, rpo[i + 1], i + 1, "implicit-fallthrough");
                }
                continue;
            }
            ...
            if (term->mnemonic == M::OP_BRANCH) {
                ...
                if (term->target_block && key_to_index.contains(*term->target_block)) {
                    add_source_edge(
                        g, node, key, *term->target_block, key_to_index[*term->target_block],
                        "branch"
                    );
                }
                ...
                if (term->target_block && labels.contains(*term->target_block)) {
                    ...
                    node.terminal =
                        new (ctx) clang::GotoStmt(labels.at(*term->target_block), loc, tgt_loc);
                    builder.TrackTerminalStmt(key, *term, node.terminal);
                }
                ...
                if (not_taken != CNode::kNone) {
                    add_source_edge(
                        g, node, key, rpo[not_taken], not_taken, "conditional-not-taken"
                    );
                }
                if (taken != CNode::kNone) {
                    add_source_edge(g, node, key, rpo[taken], taken, "conditional-taken");
                }
                ...
                if (node.branch_cond && node.succs.size() == 2) {
                    ...
                    if (term->taken_block && labels.contains(*term->taken_block)) {
                        ...
                        taken_stmt =
                            new (ctx) clang::GotoStmt(labels.at(*term->taken_block), loc, ll);
                    } else {
                    ...
                    if (term->not_taken_block && labels.contains(*term->not_taken_block)) {
                        auto ll =
                            SourceLocation(ctx.getSourceManager(), *term->not_taken_block);
                        not_taken_stmt = new (ctx)
                            clang::GotoStmt(labels.at(*term->not_taken_block), loc, ll);
                    } else {
                ...
                if (!term->switch_cases.empty()) {
                    ...
                    for (const auto &sc : term->switch_cases) {
                        ...
                        if (seen_succs.insert(target_idx).second) {
                            add_source_edge(
                                g, node, key, sc.target_block, target_idx, "switch-case"
                            );
                        }
                        // Map to succ index
                        size_t succ_idx = 0;
                        ...
                        node.switch_cases.push_back(
                            SwitchCaseEntry{ sc.value, succ_idx, sc.has_exit,
                                             /*is_default=*/false }
                        );
                    }
                    ...
                    if (term->fallback_block && key_to_index.contains(*term->fallback_block)) {
                        ...
                        if (seen_succs.insert(fb_idx).second) {
                            add_source_edge(
                                g, node, key, *term->fallback_block, fb_idx, "switch-default"
                            );
                        }
                        ...
                        node.switch_cases.push_back(
                            SwitchCaseEntry{ 0, fb_succ_idx, /*has_exit=*/false,
                                             /*is_default=*/true }
                        );
                    }
                    ...
                    for (const auto &block_key : term->successor_blocks) {
                        ...
                        if (seen_succs.insert(target_idx).second) {
                            add_source_edge(
                                g, node, key, block_key, target_idx, "switch-successor-block"
                            );
                        }
                        ...
                        node.switch_cases.push_back(
                            SwitchCaseEntry{ static_cast< int64_t >(*addr), succ_idx, false }
                        );
                    }
                }

            } else if (term->mnemonic == M::OP_RETURN || term->mnemonic == M::OP_TAIL_CALL) {
        ...
        for (size_t i = 0; i < g.nodes.size(); ++i) {
            for (size_t s : g.nodes[i].succs) {
                if (s < g.nodes.size()) { g.nodes[s].preds.push_back(i); }
            }
        }
