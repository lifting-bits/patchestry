...
namespace patchestry::passes {
    ...
    void InstrumentationPass::prepare_patch_call_arguments(
        ...
        // todo (kaoudis) is this needed? pull it up
    ...
    void InstrumentationPass::prepare_contract_call_arguments(
        ...
        // Handle structured argument specifications
        for (size_t i = 0;
             i < action.action[0].arguments.size() && i < contract_func.getNumArguments();
             ++i)
        {
                    ...
                    if (auto int_type = mlir::dyn_cast< cir::IntType >(arg_type)) {
                        try {
                            ...
                            continue;
                                   ...
                                   mlir::dyn_cast< cir::PointerType >(arg_type))
                    {
                        try {
                            // Parse pointer constant (usually hex address)
                            uint64_t ptr_val = std::stoull(const_value, nullptr, 0);
    ...
    void InstrumentationPass::apply_before_patch(
        ...
        prepare_patch_call_arguments(builder, target_op, patch_func, patch, function_args);
    ...
    void InstrumentationPass::apply_contract_before(mlir::Operation *target_op, 
        const ContractInformation &contract, mlir::ModuleOp contract_module, 
        bool should_inline
    ) {
        if (target_op == nullptr) {
            LOG(ERROR) << "apply_contract_before: the passed function to be instrumented was null";
            return;
        }
        ...
        std::string contract_function_name =
            namifyFunction(spec.implementation.function_name);
        auto input_types = llvm::to_vector(target_op->getOperandTypes());
    ...
    void InstrumentationPass::apply_contract_after(mlir::Operation *target_op, 
        const ContractInformation &contract, mlir::ModuleOp contract_module, bool should_inline
    ) {
        if (target_op == nullptr) {
            LOG(ERROR) << "apply_contract_after: the passed function to be instrumented was null";
            return;
        }
